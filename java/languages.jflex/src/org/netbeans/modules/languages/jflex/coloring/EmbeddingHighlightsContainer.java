...
package org.netbeans.modules.languages.jflex.coloring;
...
/**
 * This class is inspired/adapted from org.netbeans.modules.html.editor.coloring.EmbeddingHighlightsContainer
 */
public class EmbeddingHighlightsContainer extends AbstractHighlightsContainer implements TokenHierarchyListener {
    ...
    private static final String JAVA_BACKGROUND_TOKEN_NAME = "java_embedded"; //NOI18N
    private static final String JAVA_MIME_TYPE = "text/x-java"; //NOI18N

    private AttributeSet javaBackground;
    private final Document document;
    private TokenHierarchy<? extends Document> hierarchy = null;
    private long version = 0;
    
    private final Result<FontColorSettings> lookupResult;
    private final LookupListener lookupListener;

    EmbeddingHighlightsContainer(Document document) {
        this.document = document;

        lookupResult = MimeLookup.getLookup(JflexLanguage.MIME_TYPE).lookupResult(FontColorSettings.class);
    ...
    private void refreshColorings() {
        ...
        assert !allInstances.isEmpty();
        
        FontColorSettings fcs = allInstances.iterator().next();
        Color javaBC = null;
        if (fcs != null) {
            javaBC = getColoring(fcs, JAVA_BACKGROUND_TOKEN_NAME);
        }

        javaBackground = javaBC == null ? SimpleAttributeSet.EMPTY : AttributesUtilities.createImmutable(
        ...
        private boolean _moveNext() {
            if (tokenSequenceList == null) {
                ...
                if (language != null) {
                    ...
                    if (mimeType.equals(JflexLanguage.MIME_TYPE)) {
                        ...
                        LanguagePath htmlPath = LanguagePath.get(topLevelLanguagePath, Language.find(JflexLanguage.MIME_TYPE));
                        tokenSequenceList = scanner.tokenSequenceList(htmlPath, startOffsetBoundary, endOffsetBoundary);
                    }
                } else {
                    LOG.log(Level.WARNING, "Language {0} obtained from the document mimeType property cannot be found!", mimeType); //NOI18N
                }
            }

            if (tokenSequenceList != null) {
                for (TokenSequence<?> tokenSequence : tokenSequenceList) {
                    assert tokenSequence.language().mimeType().equals(JflexLanguage.MIME_TYPE);
                    ...
                    while (tokenSequence.moveNext() && tokenSequence.offset() < endOffsetBoundary) {
                        ...
                        if (JAVA_MIME_TYPE.equals(embeddedMimeType)) {
                            try {
                                ...
                                if(eTokenSequence.moveNext()) {
                                    ...
                                    int startLO = LineDocumentUtils.getLineStart((LineDocument) document, startOffset);
                                    int endLO = LineDocumentUtils.getLineStart((LineDocument) document, endOffset);
                                    if (startLO != endLO) {
                                        //not just one line block - test boundaries
                                        if ((LineDocumentUtils.getPreviousNonWhitespace((LineDocument) document, LineDocumentUtils.getLineEnd((LineDocument) document, startOffset)) + 1) == startOffset) {
                                            //just <script-style> tag on the first line -> move start to next line
                                            startOffset = LineDocumentUtils.getLineStartFromIndex((LineDocument) document, startLO + 1);
                                        }
                                        if (LineDocumentUtils.getNextNonWhitespace((LineDocument) document, LineDocumentUtils.getLineStartFromIndex((LineDocument) document, LineDocumentUtils.getLineIndex((LineDocument) document, endLO))) == endOffset) {
                                            //just </script-style> tag on the last line -> move block end to previous line end
                                            endOffset = LineDocumentUtils.getLineStartFromIndex((LineDocument) document, endLO);
                                        }
                                    }

                                    attributeSet = embeddedMimeType.equals(JAVA_MIME_TYPE) ? javaBackground : null;
